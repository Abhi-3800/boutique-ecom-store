...
export default function ProductCard({ product }) {
  ...
  // Start carousel timer when hovered
  useEffect(() => {
    if (hovered && images.length > 1) {
      ...
      // Stop cycling and reset when not hovered
      clearTimer();
      setActiveImage(0);
    }

    // Cleanup on component unmount or when hovered changes
    return () => clearTimer();
  ...
  return (
    ...
    >
      ...
      <div className="bg-white rounded-lg shadow-md px-3 pb-3 flex flex-col w-full mx-auto -mt-5 z-20 relative max-w-full sm:max-w-xs">
        ...
        <p className="text-gray-600 text-xs mb-1 line-clamp-2 min-h-[2.5em]">{product.description}</p>
        ...
        <div className="flex items-center justify-between mt-auto">
          ...
          {swatches ? (
            <div className="flex gap-1">
              ...
              {swatches.length > 4 && (
                <span className="ml-1 text-xs text-gray-500">+{swatches.length - 4}</span>
              )}
            </div>
        ...
        <button className="w-full mt-2 py-1 rounded bg-[#b49b7f] text-white text-sm hover:bg-[#917d63] transition">
          Add to Cart
        </button>
      </div>
    </div>
